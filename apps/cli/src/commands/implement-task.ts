...
export async function implementTask(opts: ImplementTaskOptions): Promise<void> {
  ...
  if (opts.resume) {
    ...
    const taskMode: 'simple' | 'advanced' =
      'advanced';
    const allTasks = plan.phases.flatMap((p) => p.tasks);
    const completedBefore = allTasks.filter((t) => t.status === 'complete').length;
    const totalTasks = allTasks.length;
    const remaining = totalTasks - completedBefore;

    printSection(`Resuming: ${remaining} tasks remaining (concurrency: ${concurrency})`);

    // Track per-task spinners
    ...
    const { ranCount, failedCount } = await runTasksParallel(projectPath, plan, {
      ...
      onTaskStart: (task) => {
        const s = spinner(`[${task.id}] ${task.title}`);
        spinners.set(task.id, s);
      },
      onProgress: (taskId, msg) => {
        spinners.get(taskId)!.text = `[${taskId}] ${msg}`;
      },
      onTaskDone: (task, result) => {
        const s = spinners.get(task.id);
        if (!s) return;
        if (result.success) {
          s.succeed(`[${task.id}] ${task.title} (${result.toolCallCount} tool calls)`);
        } else {
          s.fail(`[${task.id}] ${task.title} — ${result.summary.slice(0, 60)}`);
        }
      },
    });
  ...
  const plan: TaskPlan = JSON.parse(await readFile(tasksPath, 'utf8')) as TaskPlan;
  const taskMode: 'simple' | 'advanced' = 'advanced';
  ...
  if (!task) throw new Error(`Task "${opts.task}" not found in harness/tasks.json`);
  ...
  const result = await runTask(projectPath, task, plan, (msg) => {
    loopSpinner.text = msg;
  }, taskMode, undefined, undefined, undefined, undefined, undefined, opts.model, undefined, undefined, opts.reconciliationReport);

  // task object is mutated by runTask; sync back into plan for final write
  ...
  if (result.filesModified.length > 0) {
    ...
    result.filesModified.forEach((f) => console.log(c.path(f)));
  }
  ...
  if (tokenBreakdownLines.length > 0) {
    ...
    console.log(`total: in ${formatTokenCount(result.tokensUsed.input)}, out ${formatTokenCount(result.tokensUsed.output)}, cached ${formatTokenCount(result.tokensUsed.cached)}`);
