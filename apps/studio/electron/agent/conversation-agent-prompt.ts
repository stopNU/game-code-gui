...
export function buildConversationAgentPrompt(args: BuildPromptArgs): string {
  ...
  if (provider === 'codex') {
    ...
    lines.push(
      ...
      ...(modelArg !== null
        ? [`When invoking \`implement_task\`, ALWAYS pass \`model: "${modelArg}"\` — that is the model the user has selected in Studio for this conversation, and the spawned task runner must honor it.`]
        : []),
    );
  }
