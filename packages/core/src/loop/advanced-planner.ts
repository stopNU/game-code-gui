...
function ensureMilestoneScenes(
  ...
  if (milestoneScenes.length > 0) {
    return milestoneScenes.map((scene): MilestoneSceneDefinition => {
...
export async function createAdvancedPlan(
  ...
  const firstResponse = await model.invoke(
    [new SystemMessage(ADVANCED_DESIGNER_SYSTEM_PROMPT), new HumanMessage(userMessage)],
    { callbacks },
  );
  ...
  const retryResponse = await model.invoke([
    ...
    new AIMessage(text),
    new HumanMessage(
      `Your response failed schema validation. Fix these errors and return the corrected JSON only:\n` +
      errorSummary,
    ),
  ]);
