...
import type { PhaserProject } from '../types/project.js';
...
export async function scaffoldGame(opts: ScaffoldOptions): Promise<PhaserProject> {
  const { outputPath, plan } = opts;

  await ensureDir(outputPath);

  const template = selectTemplate(plan.genre);
  await copy(template.templateDir, outputPath, { overwrite: true });

  const project: PhaserProject = {
    id: toKebab(plan.gameTitle),
    path: outputPath,
  ...
  const hbsContext = buildHandlebarsContext(plan, project);
  ...
  await scaffoldAdvancedExtras(outputPath, plan, project, opts.preprocessedBrief);
